...
import { FaCheckCircle, FaExclamationCircle, FaEye, FaNotesMedical, FaUser } from "react-icons/fa";
...
const PredictionCard = ({
  ...
  const diagnosisColorMap = {
    ...
    green: ["NO_DR", "NO_AMD", "NO_RVO", "NO_GLAUCOMA", "NORMAL"],
  };
  const getDiagnosisStyle = diagnosis => {
  ...
  const maxConfidence = Array.isArray(confidence)
    ...
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition-shadow">
      ...
      <div className="mb-4">
        <div className="text-lg font-semibold text-gray-800 flex items-center gap-2">
          {icon} Diagnosis:{" "}
          <span className={`text-${color}-500`}>{prediction.prediction.label}</span>
        </div>
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div>{prediction.patientId}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Name</div>
                <div>{prediction.patientDetails.fullName}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Age</div>
                <div>{prediction.patientDetails.age}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Gender</div>
                <div>{prediction.patientDetails.gender}</div>
              </div>
            </div>
          </div>
